...
# Set DATABASE_URL to use Postgres; leave unset for SQLite-only local dev.
...
def _make_engine():
    ...
    # Try PostgreSQL when explicitly configured
    if _POSTGRES_URL and _POSTGRES_URL.startswith("postgresql"):
        try:
            eng = create_engine(
                _POSTGRES_URL,
                echo=False,
